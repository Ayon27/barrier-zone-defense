    ...
    def advv_to_tensor(self):
        ...
        three_d_list = [
            [
                [
                    self.adv_v[
                        channel * self.size_x * self.size_y + x * self.size_y + y
                    ]
...
class Iter:
    ...
    def mutation(
        ...
        if self.chosen_v >= 0:
            ...
            self.old_vbest.Rmax, self.old_vbest.Rmin = (
                self.offspringVs[self.chosen_v].Rmax,
                self.offspringVs[self.chosen_v].Rmin,
            )
            for vi in range(self.offspringN):
    ...
    def compare_directions_usingADB(
        self, model, original_image, label, aim_r, maxIters, binaryM
    ):
        ...
        while high - low >= 1e-5:  # comparison loop
            ADB = datatools.next_ADB(
                low, high, aim_r, self.old_vbest.Rmax, binaryM
            )  # guess next ADB using rho(r)
...
def ATK_ADBA(
    ...
    while (query < budget) and (ITERATION.old_vbest.Rmax > aim_r):
        ...
        for i, bi in enumerate(blocks[block_iter - 1]):
            ...
            query_plus = ITERATION.mutation(
                ...
                aim_r,
                tolerance_binary_iters,
                blocks_i[offspringN * i : offspringN * (i + 1)],
                binaryM,
            )
    ...
    return (
        ...
        adversarial_image,
        adversarial_v,
    )  # added adversarial_v for vbest return
...
def ADBA_Attack(
    ...
    # Run ATK_ADBA
    success_int, queries, iter_n, Rbest, adv_image, Vbest = (
        ATK_ADBA(  # return vbest to use to find noisy image at epsilon in best v direction
            model=model,
            device=device,
