...
def RaySAttack(device, model, epsMax, queryLimit, cleanLoader):
    ...
    n = xClean.shape[0]
    for i in range(n):
        xAdvCurrent, _, dist, _ = rayS.attack_hard_label(
            xClean[i].to(device),
            yClean[i].to(device),
            target=None,
        ...
        if dist > epsMax:
            ...
            xAdv[i] = torch.clamp(xAdv[i], 0.0, 1.0)
        else:
            xAdv[i] = xAdvCurrent
        if (i + 1) % 100 == 0 or (i + 1) == n:
            print(f"  [{i + 1}/{n}] samples attacked")
...
class RayS(object):
    ...
    def get_xadv(self, x, v, d, lb=0.0, rb=1.0):
        # out = x + d * v
    ...
    def __call__(self, data, label, target=None, seed=None, query_limit=10000):
        return self.attack_hard_label(
            data, label, target=target, seed=seed, query_limit=query_limit
        )
