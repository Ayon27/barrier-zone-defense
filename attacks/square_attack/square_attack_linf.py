...
def SquareAttackLinf_Wrapper(
    ...
    print(
        f"Attack mode: {'Targeted' if targeted else 'Untargeted'}, Loss type: {loss_type}, n_queries: {n_queries.shape}, n_iters: {n_iters}, x_tensor: {x_tensor.shape}"
    )
...
def square_attack_linf(
    ...
    x_best = np.clip(
        square_attack_utils.quantize_numpy(x + init_delta), min_val, max_val
    ...
    for i_iter in range(n_iters - 1):
        ...
        x_new = np.clip(
            square_attack_utils.quantize_numpy(x_curr + deltas), min_val, max_val
        )  # quantization applied
        ...
        mean_nq, mean_nq_ae, median_nq_ae = (
            np.mean(n_queries),
        ...
        # print(
        #     "{}: acc={:.2%} acc_corr={:.2%} avg#q_ae={:.2f} med#q={:.1f}, avg_margin={:.2f} (n_ex={}, eps={:.3f}, {:.2f}s)".format(
        #         i_iter + 1,
        #         acc,
        #         acc_corr,
        #         mean_nq_ae,
