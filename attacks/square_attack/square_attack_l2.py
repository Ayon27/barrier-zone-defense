...
def square_attack_l2(
    model, device, x, y, corr_classified, eps, n_iters, p_init, targeted, loss_type
):
    ...
    for counter in range(h // s):
        ...
        for counter2 in range(w // s):
            ...
            ] += square_attack_utils.meta_pseudo_gaussian_pert(s).reshape(
                [1, 1, s, s]
            ) * np.random.choice(
    ...
    x_best = np.clip(
        square_attack_utils.quantize_numpy(
            x
            + delta_init
            / np.sqrt(np.sum(delta_init**2, axis=(1, 2, 3), keepdims=True) + 1e-10)
            * eps
        ),
        min_val,
        max_val,
    ...
    for i_iter in range(n_iters):
        ...
        curr_norms_image = np.sqrt(
            np.sum((x_best_curr - x_curr) ** 2, axis=(1, 2, 3), keepdims=True) + 1e-10
        )
        mask_2 = np.maximum(new_deltas_mask, new_deltas_mask_2)
        norms_windows = np.sqrt(
            np.sum((delta_curr * mask_2) ** 2, axis=(2, 3), keepdims=True) + 1e-10
        )

        ### create the updates
        new_deltas = np.ones([x_curr.shape[0], c, s, s])
        new_deltas = new_deltas * square_attack_utils.meta_pseudo_gaussian_pert(
            s
        ).reshape([1, 1, s, s])
        ...
        x_new = (
            x_curr
            + delta_curr
            / np.sqrt(np.sum(delta_curr**2, axis=(1, 2, 3), keepdims=True) + 1e-10)
            * eps
        )
        # x_new = np.clip(x_new, min_val, max_val)
        x_new = np.clip(
            square_attack_utils.quantize_numpy(x_new), min_val, max_val
        )  # quantization applied
        ...
        margin = square_attack_utils.loss(
            y_curr, logits, targeted, loss_type="margin_loss"
        )
        ...
        mean_nq_ae = (
            np.mean(n_queries[margin_min <= 0])
            if np.any(margin_min <= 0)
            else np.mean(n_queries)
        )
        median_nq = np.median(n_queries)
        median_nq_ae = (
            np.median(n_queries[margin_min <= 0])
            if np.any(margin_min <= 0)
            else np.median(n_queries)
        )

        time_total = time.time() - time_start
        print(
            "{}: acc={:.2%} acc_corr={:.2%} avg#q_ae={:.1f} med#q_ae={:.1f} s={}->{}, n_ex={}, {:.0f}s, loss={:.3f}, max_pert={:.1f}, impr={:.0f}".format(
                i_iter + 1,
                acc,
                acc_corr,
                mean_nq_ae,
        ...
        if acc == 0:
            ...
            print(
                "Maximal norm of the perturbations: {:.5f}".format(
                    np.amax(curr_norms_image)
                )
            )
            break
