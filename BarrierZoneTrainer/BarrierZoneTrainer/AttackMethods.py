...
def LoadBARZ8(modelDir):
    ...
    for i in range(0, len(scaleFactors)):
        ...
        checkpoint = torch.load(
            modelDir + "ModelResNet56BARZ-" + str(scaleFactors[i]) + ".th"
        )
        model.load_state_dict(checkpoint["state_dict"])
        model.eval()
        # Append the current model to the model list
        modelListPlus.append(
            ModelPlus.ModelPlus(
                "ModelResNet56BARZ-" + str(scaleFactors[i]),
                model,
                device,
                inputImageSize,
                inputImageSize,
    ...
    defenseBARZ = BarrierZoneDefense.BarrierZoneDefense(
        modelListPlus, numClasses, threshold
    )
    return defenseBARZ
