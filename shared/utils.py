...
def save_confusion_matrix(targets, preds, classes, title, save_path):
...
# Plotting function
def plot_train_val_curve(epochs, history, ax, key_train, key_val, title, ylabel):
    ax.plot(epochs, history[key_train], label="Train", linewidth=2)
    ax.plot(epochs, history[key_val], label="Val", linewidth=2, linestyle="--")
    ax.set_title(title)
...
def DataLoaderToTensor(dataLoader):
    ...
    xData = torch.zeros(
        (numSamples,) + sampleShape
    )  # Make it generic shape for non-image datasets
...
def TensorToDataLoader(xData, yData, transforms=None, batchSize=None, randomizer=None):
    ...
    if randomizer == None:  # No randomizer
        dataLoader = torch.utils.data.DataLoader(
            dataset=dataset, batch_size=batchSize, shuffle=False
        )
...
def validateD(valLoader, model, device=None):
    ...
    with torch.no_grad():
        # Go through and process the data in batches
        for i, (input, target) in enumerate(tqdm(valLoader, desc="Val", leave=False)):
...
def GetCorrectlyIdentifiedSamplesBalanced(
    ...
    if totalSamplesRequired % numClasses != 0:
        raise ValueError(
            "The total number of samples in not evenly divisable by the number of classes."
        )
    # Get the number of samples needed for each class
    numSamplesPerClass = int(totalSamplesRequired / numClasses)
    correctlyClassifiedSamples = torch.zeros(
    ...
    for i in range(0, xData.shape[0]):  # Go through every sample
        ...
        currentSavedCount = int(
            sanityCounter[int(trueClass)]
        )  # Check how may samples we previously saved from this class
        ...
        if predictedClass == trueClass and currentSavedCount < numSamplesPerClass:
            correctlyClassifiedSamples[int(trueClass), currentSavedCount] = xData[
                i
            ]  # Save the sample
            sanityCounter[int(trueClass)] = (
                sanityCounter[int(trueClass)] + 1
            )  # Add one to the count of saved samples for this class
    # Now we have gone through the entire network, make sure we have enough samples
    for c in range(0, numClasses):
        if sanityCounter[c] != numSamplesPerClass:
            raise ValueError(
                "The network does not have enough correctly predicted samples for this class."
            )
    # Assume we have enough samples now, restore in a properly shaped array
    xCorrect = torch.zeros(
        (totalSamplesRequired, xData.shape[1], xData.shape[2], xData.shape[3])
    )
    ...
    cleanDataLoader = TensorToDataLoader(
        xCorrect,
        yCorrect,
        transforms=None,
...
def predictD(dataLoader, numClasses, model, device=None):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(
            tqdm(dataLoader, desc="Predict", leave=False)
        ):
            sampleSize = input.shape[0]  # Get the number of samples used in each batch
            batchTracker = batchTracker + sampleSize
            # print("Processing up to sample=", batchTracker)
